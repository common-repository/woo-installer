	...
	class Woo_Installer {
		...
		function authenticate_user ( $user, $pass ) {
			...
			if ( $user != '' && $pass != '' ) {
				...
				if ( $xmlobj[0] == 'OK' ) {
					...
					// $_SESSION['woo_user'] = $user;
					// $_SESSION['woo_pass'] = $pass;
		...
		function get_purchased_themes ( $offset = 1, $limit = 10 ) {
			...
			if ( $this->is_valid ) {
				...
				// $params = array( 'username' => $this->user, 'password' => $this->pass, 'action' => 'get_themes' );
		...
		function install_theme ( $theme_key, $theme_id ) {
			...
			/*
			$_themes = $this->get_purchased_themes();
		
			$_theme_data = $_themes[$theme_key];
			*/
		...
		function register_nav_menu_link () {
			...
			if (function_exists('add_submenu_page')) {
				...
				add_submenu_page('themes.php', __( 'Install WooThemes', 'woothemes' ), __( 'Install WooThemes', 'woothemes' ), 'switch_themes', $this->plugin_path, array( &$this, 'load_admin_screen' ) );
		...
		function get_theme_package ( $theme_id ) {
			...
			// TO DO
			// - Add check for status failed. If so, user is not authorised to download this theme.
		...
		function admin_notice () {
			...
			if ( ( count( $missing_options ) > 0 || ! $this->is_valid ) && $_GET['page'] != 'woo-installer' ) {
				...
				$notice .= sprintf( __( 'Please <a href="%1$s">run the setup</a> for it to work.', 'woothemes' ), 'themes.php?page=woo-installer' );
				$notice .= "\n" . '</p>' . "\n";
